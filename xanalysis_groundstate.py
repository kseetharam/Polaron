...
if __name__ == "__main__":
    ...
    quad1 = axes[0].pcolormesh(kzg_Sph, kxg_Sph, PhDen_Sph[:-1, :-1], norm=colors.LogNorm(vmin=np.abs(np.min(PhDen_Sph)), vmax=np.max(PhDen_Sph)), cmap='GnBu')
    quad1m = axes[0].pcolormesh(kzg_Sph, -1 * kxg_Sph, PhDen_Sph[:-1, :-1], norm=colors.LogNorm(vmin=np.abs(np.min(PhDen_Sph)), vmax=np.max(PhDen_Sph)), cmap='GnBu')
    fig.colorbar(quad1, ax=axes[0], extend='both')
    quad2 = axes[1].pcolormesh(kzLg_ky0slice, kxLg_ky0slice, PhDenLg_ky0slice[:-1, :-1], norm=colors.LogNorm(vmin=np.abs(np.min(PhDen_Sph)), vmax=np.max(PhDen_Sph)), cmap='GnBu')
    ...
    fig2, ax2 = plt.subplots()
    quad3 = ax2.pcolormesh(zLg_y0slice_interp, xLg_y0slice_interp, nxyz_y0slice_interp[:-1, :-1], norm=colors.LogNorm(vmin=np.abs(np.min(nxyz_y0slice_interp)), vmax=np.max(nxyz_y0slice_interp)), cmap='GnBu')
    ...
    fig3, ax3 = plt.subplots()
    quad4 = ax3.pcolormesh(zLg_y0slice_interp, xLg_y0slice_interp, na_xyz_y0slice_interp[:-1, :-1], norm=colors.LogNorm(vmin=np.abs(np.min(na_xyz_y0slice_interp)), vmax=np.max(na_xyz_y0slice_interp)), cmap='GnBu')
