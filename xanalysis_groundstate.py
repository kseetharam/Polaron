...
if __name__ == "__main__":
    ...
    fig1, ax1 = plt.subplots()
    quad1 = ax1.pcolormesh(kzg_Sph, kxg_Sph, PhDen_Sph[:-1, :-1], norm=colors.LogNorm(vmin=np.abs(np.min(PhDen_Sph)), vmax=np.max(PhDen_Sph)), cmap='GnBu')
    quad1m = ax1.pcolormesh(kzg_Sph, -1 * kxg_Sph, PhDen_Sph[:-1, :-1], norm=colors.LogNorm(vmin=np.abs(np.min(PhDen_Sph)), vmax=np.max(PhDen_Sph)), cmap='GnBu')
    ...
    fig2, ax2 = plt.subplots()
    quad2 = ax2.pcolormesh(kzLg_ky0slice, kxLg_ky0slice, PhDenLg_ky0slice[:-1, :-1], norm=colors.LogNorm(vmin=np.abs(np.min(PhDen_Sph)), vmax=np.max(PhDen_Sph)), cmap='GnBu')
    ...
    fig3, ax3 = plt.subplots()
    quad3 = ax3.pcolormesh(zLg_y0slice_interp, xLg_y0slice_interp, nxyz_y0slice_interp[:-1, :-1], norm=colors.LogNorm(vmin=np.abs(np.min(nxyz_y0slice_interp)), vmax=np.max(nxyz_y0slice_interp)), cmap='GnBu')
    ...
    quad4 = ax4.pcolormesh(zLg_y0slice_interp, xLg_y0slice_interp, na_xyz_y0slice_interp[:-1, :-1], norm=colors.LogNorm(vmin=np.abs(np.min(na_xyz_y0slice_interp)), vmax=np.max(na_xyz_y0slice_interp)), cmap='GnBu')
