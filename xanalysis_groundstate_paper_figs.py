...
if __name__ == "__main__":
    ...
    # ax_gsE.plot(Pinf_Vals / (mI * nu), Einf_Vals, 'k-')
    # ax_gsE.set_xlabel(r'$P$ [$m_{I}c$]', fontsize=labelsize)
    # ax_gsE.set_ylabel(r'$E$', fontsize=labelsize)
    # ax_gsE.set_ylim([1.1 * np.min(Einf_Vals), -0.5])
    # ax_gsE.set_xlim([0, 2.0])
    # ax_gsE.yaxis.set_major_locator(plt.MaxNLocator(2))

    # Pcrit = Pinf_Vals[np.argmin(np.gradient(Einf_2ndderiv_Vals)) - 0]
    # ax_gsE.axvline(x=Pcrit / (mI * nu), linestyle=':', color='#bf9005', lw=2)
    ...
    # gs2 = fig1.add_gridspec(nrows=2, ncols=1, bottom=0.13, top=0.94, left=0.67, right=0.99, height_ratios=[1, 1], hspace=0.5)
    ...
    # ax_PD.plot(aIBi_interpVals / xi, mE_interpVals, color='#ba9e88', linestyle='dashed')
    ...
    # con_sup = ConnectionPatch(xyA=(supDist_coords[0], supDist_coords[1]), xyB=(0, 0.52), coordsA="data", coordsB="data", axesA=ax_PD, axesB=ax_supDist, color='#8f1402', linestyle='dotted', lw=0.5)
    # con_sub = ConnectionPatch(xyA=(subDist_coords[0], subDist_coords[1]), xyB=(0, 0.35), coordsA="data", coordsB="data", axesA=ax_PD, axesB=ax_subDist, color='#8f1402', linestyle='dotted', lw=0.5)
